const HEROES = [
  ...
  },
  {
    ...
    image: {
      filename:
        "https://terrigen-cdn-dev.marvel.com/content/prod/1x/hawkeye_ons_crd_01.jpg",
...
/**
 * 1. Function for card creation - создание контента для карточки
 * 2. function for generating content within card
 * 3. get data from filters form
 * - get link to the form
 * - add listener
 * - get form elements, values from inputs
 * 4. handle form submit and regenerate content
 */
...
//first point is done
// дальше - необходимо получить ссылку на враппер, коробочку куда иы будем складывать контент
...
const generateContent = (array) => {
  ...
  // Другой более элегантный метод вместо forEach - метод reduce - тогда будет
  // const data = array.reduce((acc, element) => {
  // acc += createCardContent(element);
  //}, '');

  //тут мы проходимся по всему массиву и добавляем элементы в нашу пустую строку
  //плюс-равно потому что надо записать data = data + createCardContent - это заменяет
...
//теперь нужно прикрутить фильтры после
// обработки события submit для формы и вытащить данные
